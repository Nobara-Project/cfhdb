...
pub struct CfhdbUsbDevice {
    ...
    pub available_profiles: Rc<RefCell<Option<Vec<CfhdbUsbProfile>>>>,
}
impl CfhdbUsbDevice {
    ...
    fn get_manufacturer(busid: &str) ->  Result<String, std::io::Error> {
        ...
        if device_manufacturer_path.exists() {
            ...
            Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Manufacturer file could not be found!"))
        }
    }     
    ...
    fn get_product(busid: &str) ->  Result<String, std::io::Error> {
        ...
        if device_product_path.exists() {
            ...
            Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Product file could not be found!"))
        }
    }        
    ...
    pub fn get_devices() -> Option<Vec<Self>> {
        ...
        for iter in usb_devices.iter() {
            ...
            let item_sysfs_busid = Self::get_sysfs_id(item_bus_number, item_address).unwrap_or("???".to_owned());//format!("{}-{}-{}", iter.bus_number(), iter.port_number(), iter.address());
        ...
        for device in devices.iter() {
            ...
            let found = uniq_devices.iter().any(|x: &Self| {
                device.sysfs_busid == x.sysfs_busid
            });
